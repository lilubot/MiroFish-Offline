...
def get_generate_status():
    ...
    try:
        ...
        # If simulation_id provided, check if completed report already exists
        if simulation_id:
            existing_report = ReportManager.get_report_by_simulation(simulation_id)
            if existing_report and existing_report.status == ReportStatus.COMPLETED:
                return jsonify({
...
def get_report_sections(report_id: str):
    ...
    Frontend can poll this endpoint to get generated section content without waiting for complete report
...
def check_report_status(simulation_id: str):
    ...
    try:
        ...
        # Unlock interview only after report completes
...
def get_agent_log(report_id: str):
    """
    Get detailed execution log for Report Agent

    Retrieves each action step during report generation in real-time, including:
    - Report start, planning start/completion
    - Each section start, tool calls, LLM responses, completion
    - Report completion or failure

    Query parameters:
        from_line: Start reading from line number (optional, default 0, for incremental fetching)
...
def get_console_log(report_id: str):
    ...
    Unlike agent-log interface which returns structured JSON logs, this is plain text console-style log.
